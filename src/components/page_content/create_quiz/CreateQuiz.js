...
import {
  createNewCategory,
  getAllCategories,
} from "../../../services/api_categories";

function CreateQuiz() {
  const { setQuizId, setRoundNum, setCurrentRoundNum } =
  ...
  const checkForIdenticalCategory = async (e) => {
    try {
      ...
      const category = categories.find(
        (category) => category.name === e.target[1].value.toLowerCase()
      );
  ...
  const handleQuizCreate = async (e) => {
    try {
      ...
      form.append(
        "date_to_signup",
        e.target[4].value + " " + e.target[5].value + ":00"
      );
    ...
    <Fragment>
      <section className="flex flex-col items-center  w-full sm:block h-3/4 sm:w-2/5 md:w-[40%]">
        ...
        <button
          ...
          type="submit"
          className="submit-btn cursor-pointer standard-button bg-main-dark-theme text-text-light-color w-36 h-10 p-2 text-center rounded-md mt-6 after:!bg-main-theme"
        >
          Dalje
        </button>
      </section>
      <section className="sm:w-2/5 md:w-[35%] lg:w-[30%] sm:pt-14 md:pt-[12%] lg:pt-[8%] flex flex-col items-center sm:block">
        <div className="h-48 md:h-52 w-3/4 sm:w-full lg:h-64 bg-gray-200 flex justify-center items-center flex-col gap-10 !border-3 !border-black border-solid">
          ...
          <button
            id="img-load-button"
            className="standard-button bg-main-theme w-36 h-10 p-2 rounded-md text-text-color"
          >
            Učitaj sliku
          </button>
        </div>
